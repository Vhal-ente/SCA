...
export const playerSlug = (value = "") => value.toLowerCase().trim().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "");
...
export const getPlayerProfile = (identifier="") => { const normalized=playerSlug(identifier); return playerProfiles.find(player=>player.slug===normalized)||{slug:normalized,ign:identifier.replace(/[-_]/g," "),name:identifier.replace(/[-_]/g," "),team:"No team",role:"Player",game:"Not set",country:"Nigeria",image:null,bio:"This player has not added a public bio yet."}; };
