...
import { EventModal, ShogunGallery, ShogunHero, ShogunMark, SponsorGrid, TeamDivisionCard, UpcomingEventCard } from "@/components/Shogun";
...
export default function ShogunPage() {
  ...
  return <div className="shogun-page"><Navbar/><main>
    ...
    <section className="shogun-section shogun-surface"><div className="container"><div className="shogun-heading"><div><p className="shogun-eyebrow">Next battles</p><h2>Upcoming</h2></div></div><div className="events-row">{shogunEvents.map(event => <UpcomingEventCard key={event.title} event={{ ...event, onSelect: setSelectedEvent }}/>)}</div></div></section>
    <section id="shogun-teams" className="shogun-section"><div className="container"><div className="shogun-heading"><div><p className="shogun-eyebrow">Competitive divisions</p><h2>Our Teams</h2></div></div><div className="division-grid">{shogunDivisions.map(division => <TeamDivisionCard key={division.slug} division={division}/>)}</div></div></section>
  ...
  </main><Footer/><EventModal event={selectedEvent} onClose={() => setSelectedEvent(null)}/></div>;
}
