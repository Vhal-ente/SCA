...
import { CategoryFilter, FeaturedArticle, NewsCard, Newsletter } from "@/components/News";
import { matchesCategory, newsArticles, newsCategories } from "@/data/news";
import "./news.css";

export default function NewsPage() {
  const [activeCategory, setActiveCategory] = useState("All");
  const articles = useMemo(() => newsArticles.slice(1).filter(article => matchesCategory(article, activeCategory)), [activeCategory]);
  ...
  return <div className="news-page"><Navbar/><main>
    ...
    <section className="news-section"><div className="container"><p className="section-label">Featured news</p><FeaturedArticle article={newsArticles[0]}/></div></section>
